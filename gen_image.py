...
# Grab from https://github.com/cyxx/extract_android_ota_payload
# FlyGoat: payload.bin to tempdir, output simg
...
def parse_payload(payload_f, partition, out_f):
  BLOCK_SIZE = 4096
  with SimgWriter.SimgWriter(out_f, blocksize=BLOCK_SIZE) as wr:
...
def main(filename, output_dir):
  ...
  if filename.endswith('.zip'):
    ...
    payload_file = open(ota_zf.extract('payload.bin', temp_dir.name), 'rb')
  else:
...
if __name__ == '__main__':
  ...
  try:
    ...
    output_dir = os.getcwd() + '/out/images'
